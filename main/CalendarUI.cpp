...
void CalendarUI::do_render(lv_obj_t* parent) {
	...
	static lv_coord_t left_cont_row_desc[] = { LV_GRID_CONTENT, LV_GRID_FR(1), LV_GRID_CONTENT, LV_GRID_FR(1), LV_GRID_CONTENT, LV_GRID_FR(1), LV_GRID_CONTENT, LV_GRID_FR(1), LV_GRID_TEMPLATE_LAST };
	...
	static lv_coord_t right_cont_row_desc[] = { LV_GRID_CONTENT, LV_GRID_FR(1), LV_GRID_CONTENT, LV_GRID_FR(1), LV_GRID_CONTENT, LV_GRID_FR(1), LV_GRID_TEMPLATE_LAST };
...
void CalendarUI::create_day(lv_obj_t* parent, int weekday, uint8_t col, uint8_t row) {
	...
	lv_obj_set_style_pad_top(label, lv_dpx(6), LV_PART_MAIN);
	...
	for (auto& event : _data.events) {
		if (event.start.year == year && event.start.month == month && event.start.day == day && event.start.has_time) {
			if (had_all_day) {
				...
				auto hor_line_cont = lv_obj_create(cont);
				reset_layout_container_styles(hor_line_cont);
				lv_obj_set_style_pad_ver(hor_line_cont, lv_dpx(2), LV_PART_MAIN);
				lv_obj_set_size(hor_line_cont, LV_PCT(100), LV_SIZE_CONTENT);
...
const char* CalendarUI::get_weekday(int weekday) {
	switch (weekday) {
	case 0: return MSG_MONDAY;
	case 1: return MSG_TUESDAY;
	case 2: return MSG_WEDNESDAY;
	case 3: return MSG_THURSDAY;
	case 4: return MSG_FRIDAY;
	case 5: return MSG_SATURDAY;
	case 6: return MSG_SUNDAY;
...
const char* CalendarUI::get_month(int month) {
	switch (month) {
	case 1: return MSG_JANUARY;
	case 2: return MSG_FEBRUARY;
	case 3: return MSG_MARCH;
	case 4: return MSG_APRIL;
	case 5: return MSG_MAY;
	case 6: return MSG_JUNE;
	case 7: return MSG_JULY;
	case 8: return MSG_AUGUST;
	case 9: return MSG_SEPTEMBER;
	case 10: return MSG_OCTOBER;
	case 11: return MSG_NOVEMBER;
	case 12: return MSG_DECEMBER;
	default: assert(false); return nullptr;
	}
}
